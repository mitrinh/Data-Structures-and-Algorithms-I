...
BinaryNode<ItemType>* BinaryNodeTree<ItemType>::randomAdd(BinaryNode<ItemType>* subTreePtr, BinaryNode<ItemType>* newNodePtr)
{
   ...
   {
      int random = rand() % 100 + 1; //creates a random number that is between 1 and 100
      BinaryNode<ItemType>* leftPtr = subTreePtr->getLeftChildPtr();
      BinaryNode<ItemType>* rightPtr = subTreePtr->getRightChildPtr();
      if (random < 51) //traverse left tree if random number is less than 51
      {
         leftPtr = randomAdd(leftPtr, newNodePtr); // repeats but starting from the left tree
         subTreePtr->setLeftChildPtr(leftPtr); // sets the left child tree   
      }
      else if (random >= 51) //traverse right tree if random number is greater than or equal to 51
      {
	 rightPtr = randomAdd(rightPtr, newNodePtr); // repeats but starting from the right tree
         subTreePtr->setRightChildPtr(rightPtr); // sets the right child tree
      } 
   }
   return subTreePtr; //adds the node to the tree
}
