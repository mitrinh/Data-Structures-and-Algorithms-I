...
int SortedListHasA<ItemType>::getPosition(const ItemType& anEntry) const
{
   ...
   int position = 1;   
   if (isEmpty()) // if list is empty, position = -1
   {
     position = -1;
     return position;
   }
   for (int i = 1; i <= getLength(); i++) // if the entry is in the list, return its position
   {
   ...
   for (int j = 1; j <= getLength(); j++) // if the entry isn't in list, find what position it would be in
   {
     if (anEntry < getEntry(j)) // puts the entry in the supposed position
       {
	 position++;
       }
   }
   position = position * -1; // change to negative since entry isn't in list
   return position;   
}  // end getPosition
